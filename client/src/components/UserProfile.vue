<template>
  <div class="bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl shadow-2xl rounded-2xl p-8 md:p-10 border border-white/20 dark:border-slate-700/50 animate-fade-in-up">
    <div class="flex justify-between items-center mb-8 flex-wrap gap-4">
      <div>
        <h1 class="text-3xl sm:text-4xl font-extrabold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
          User Profile
        </h1>
    ...
    <div v-if="serverError" class="mb-6 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 p-4 rounded-xl border border-red-200 dark:border-red-800 animate-fade-in-up">
      {{ serverError }}
    </div>
    <div v-if="successMessage" class="mb-6 text-sm text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30 p-4 rounded-xl border border-green-200 dark:border-green-800 animate-fade-in-up">
      ✅ {{ successMessage }}
    </div>

    <div v-if="user" class="bg-gradient-to-br from-slate-50 to-indigo-50 dark:from-slate-700/50 dark:to-slate-800/50 rounded-xl p-6 mb-8 border border-slate-200 dark:border-slate-700">
      <div class="space-y-4 text-base sm:text-lg">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <span class="text-indigo-600 dark:text-indigo-400 font-medium">{{ user.username }}</span>
          </div>
        </div>
        <div class="flex items-center gap-3">
          <span class="text-2xl">📧</span>
    ...
    <div v-else class="mt-8 animate-fade-in-up">
      ...
      <div class="bg-gradient-to-br from-slate-50 to-indigo-50 dark:from-slate-700/50 dark:to-slate-800/50 rounded-xl p-6 border border-slate-200 dark:border-slate-700 space-y-4">
        <div class="flex items-start gap-3">
          <span class="text-xl">👤</span>
          <div>
            <strong class="font-semibold text-slate-700 dark:text-slate-300 block mb-1">First Name</strong>
            <span v-if="profile?.firstName" class="text-indigo-600 dark:text-indigo-400 font-medium">{{ profile.firstName }}</span>
        ...
        <div class="flex items-start gap-3">
          <span class="text-xl">👤</span>
          <div>
            <strong class="font-semibold text-slate-700 dark:text-slate-300 block mb-1">Last Name</strong>
            <span v-if="profile?.lastName" class="text-indigo-600 dark:text-indigo-400 font-medium">{{ profile.lastName }}</span>
            <span v-else class="italic text-slate-500 dark:text-slate-400">Not set</span>
          </div>
        </div>
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <span v-if="profile?.bio" class="text-indigo-600 dark:text-indigo-400 font-medium">{{ profile.bio }}</span>
            <span v-else class="italic text-slate-500 dark:text-slate-400">Not set</span>
          </div>
        </div>
